...
def process_telegram_contact_to_two_p(driver):
    driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()))
    try:
...
def process_telegram_contact_to_three_p(driver):
    driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()))
    try:
        with open(path, 'r') as file:
            data = json.load(file)
...
elif process_telegram_contact_to_two_p():
    time.sleep(120)
